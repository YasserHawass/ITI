...
class Stack2{
    ...
    public:
        ...
        friend void ViewContent_c(Stack2 x); // overloading without _c, will cause ambiguity error, due to confusion
};
...
void Stack2::operator=(Stack2 s){
    delete[] this->st; // so we don't have any left over space in heap.
    this->tos = s.tos;
    this->size = s.size;
